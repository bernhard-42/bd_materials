"""Wood materials (orthotropic) -- accessed by species functions.
...
(MDF/OSB) are ~isotropic in-plane, so their perpendicular modulus ~ parallel.
    ...
    from bd_materials import wood
    wood.oak()          # a WoodMaterial
    wood.all()          # every wood instance
"""
...
def ash() -> WoodMaterial:
    return _ASH


def beech() -> WoodMaterial:
    return _BEECH


def birch() -> WoodMaterial:
    return _BIRCH


def maple() -> WoodMaterial:
    return _MAPLE


def oak() -> WoodMaterial:
    return _OAK


def walnut() -> WoodMaterial:
    return _WALNUT


def spruce() -> WoodMaterial:
    return _SPRUCE


def mdf() -> WoodMaterial:
    return _MDF


def osb() -> WoodMaterial:
    return _OSB


_ALL = (_ASH, _BEECH, _BIRCH, _MAPLE, _OAK, _WALNUT, _SPRUCE, _MDF, _OSB)


def all() -> tuple[WoodMaterial, ...]:
    """Every curated wood instance (for tooling / the self-check)."""
    return _ALL
